...
if [ -f ".env" ] && grep -q "TELEGRAM_BOT_TOKEN" .env; then
    ...
    echo -e "  ${RED}✗ TELEGRAM_BOT_TOKEN not found or .env file does not exist${NC}"
fi

# Check file permissions
echo ""
echo -e "${BLUE}[6/7] Checking file permissions${NC}"
...
for script in "${SCRIPTS[@]}"; do
    if [ -f "$script" ]; then
        if [ -x "$script" ]; then
            echo -e "  ${GREEN}✓ $script has execution permission${NC}"
        else
            echo -e "  ${RED}✗ $script lacks execution permission${NC}"
...
if [ $SCRIPT_ISSUES -gt 0 ]; then
    echo -e "  ${YELLOW}Do you want to set execution permissions for all scripts? (y/n)${NC}"
    ...
    if [ "$fix_permissions" = "y" ] || [ "$fix_permissions" = "Y" ]; then
        for script in "${SCRIPTS[@]}"; do
            if [ -f "$script" ] && [ ! -x "$script" ]; then
                chmod +x "$script"
                echo -e "  ${GREEN}✓ Execution permission set for $script${NC}"
            fi
        done
    fi
fi
...
# Check disk space
echo ""
echo -e "${BLUE}[7/7] Checking disk space${NC}"
