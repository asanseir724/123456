    ...
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, 'r') as f:
                file_content = f.read()
                if file_content.strip():  # Make sure file is not empty
                    _config = json.loads(file_content)
                    logger.info("Configuration loaded from file successfully")
                    # Create a backup of the valid config file
                    try:
        ...
        _config = DEFAULT_CONFIG
        _save_config()  # Ensure default configuration is saved
        
def _save_config():
    """Save current configuration to file"""
    try:
        # First try to create a backup of the existing file
        if os.path.exists(CONFIG_FILE):
            backup_file = f"{CONFIG_FILE}.backup"
            try:
